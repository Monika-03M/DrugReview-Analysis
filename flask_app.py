...
def perform_task():
    ...
    result_output = (f"Predicted Condition: {condition_output}\n"
                     f"Predicted Rating: {rating_output}\n"
                     f"Side Effects: {side_effects_output}\n"
                     f"{topic_output}\n"
                     f"Sentiment score: {results['sentiment_score']:.2f}\n"
                     f"Sentiment category: {results['sentiment_category']}\n")
    ...
    if results['sentiment_category'] == 'Negative' and results['sentiment_distribution']:
        ...
        for category, count in results['sentiment_distribution'].items():
            sentiment_distribution_output += (
                f"{category}: {count} "
                f"({count/sum(results['sentiment_distribution'].values())*100:.1f}%)\n"
            )
        result_output += sentiment_distribution_output
        #if img_data:
            #result_output += f"Visualization image (base64): {img_data}\n"
    else:
...
#             session['logged_in'] = True
#             return jsonify({'redirect': url_for('index')}) # Return URL to redirect
#         else:
#             return jsonify({'result': 'Invalid OTP or Email'})
#     return jsonify({'result': 'Invalid OTP or Email'})
